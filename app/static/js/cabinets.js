...
async function fillScheduleDays() {
    ...
    let siteTitleCorpData = query_params.corps;

    let siteTitleName = document.querySelector('.site-title-inner');
    siteTitleName.innerHTML = siteTitleNameData;

    let siteTitleCorp = document.querySelector('.site-title-inner-corp');
    siteTitleCorp.innerHTML = siteTitleCorpData;
    // FILL
}

async function fillCabinets() {
    let id = query_params.cabinet_id;
    let cabinetsData = await getCabinets(id);
    ...
    for (let i = 0; i < cabinetsData.length; i++) {
        pairs[i].innerHTML = cabinetsData[i].name;
        cabinets[i].innerHTML = cabinetsData[i].group;
        teachers[i].innerHTML = cabinetsData[i].teacher;

        if (cabinetsData[i].is_replace) {
...
async function getCabinets(id) {
    let response = await fetch(`${SCHEDULE_API}/cabinet/${id}`);
    return await response.json();
}
